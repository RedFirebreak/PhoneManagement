    ...
    if ($devicescsv.Where({$PSItem.devicename -eq "$devicename"}).deviceip ){
        ...
        {
            Clear-Host
            Write-Output "Please select what you want to do with $devicename $specifieddeviceconnect"
            Write-Host "1: Press '1' to connect to the device "
            Write-Host "2: Press '2' to view device screen using scrcpy"
            Write-Host "3: Press '3' to send a reboot signal"
            Write-Host "4: Press '4' to install"$h.Get_Item("AppInstall")
            Write-Host "5: Press '5' for info on device (Temp, CPU)"
            Write-Host " "
